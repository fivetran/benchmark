     ...
     AS (SELECT d_year,
         ...
         FROM   (SELECT d_year,
                 ...
                 FROM   {{source('src__tpc_ds', 'catalog_sales')}}
                        ...
                        JOIN {{source('src__tpc_ds', 'date_dim')}}
                          ON d_date_sk = cs_sold_date_sk
                        LEFT JOIN {{source('src__tpc_ds', 'catalog_returns')}}
                 ...
                 FROM   {{source('src__tpc_ds', 'store_sales')}}
                        ...
                        LEFT JOIN {{source('src__tpc_ds', 'store_returns')}}
                 ...
                 FROM   {{source('src__tpc_ds', 'web_sales')}}
                        ...
                        LEFT JOIN {{source('src__tpc_ds', 'web_returns')}}
         ...
         GROUP  BY d_year,
                   ...
                   i_category_id,
                   i_manufact_id)
SELECT {{ 'TOP 100' if target.name == 'synapse' }} prev_yr.d_year                        AS prev_year,
